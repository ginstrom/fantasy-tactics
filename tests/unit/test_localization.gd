...
func test_translation_keys_resolve_to_expected_english_copy() -> void:
	...
	assert_eq(tr("debug.party_manager"), "Parties")
	...
	assert_eq(tr("unit_details.mp") % [1, 3], "MP: 1 / 3")
	assert_eq(tr("unit_details.heal_party_member"), "Heal party member")
	assert_eq(tr("unit_details.heal_unavailable"), "No legal, affordable heal target is available.")
