...
func test_buildings_shows_the_title_and_the_back_action() -> void:
	...
	assert_eq(screen.get_node("Center/VBox/Title").text, "buildings.title")
...
## table has exactly one row: Guild Hall.
func test_buildings_table_uses_the_documented_columns_and_has_one_row() -> void:
	var screen: Control = BuildingsScene.instantiate()
	add_child_autofree(screen)
	var tree: Tree = screen.get_node("Center/VBox/BuildingTable/Tree")

	assert_eq(tree.columns, 1)
	assert_eq(tree.get_column_title(0), "Name")
	assert_eq(UiTestHelpers.tree_row_values(tree, 0), ["Guild Hall"])
