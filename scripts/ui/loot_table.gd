...
## Stores explicitly uses the direct action bar.
##
...
func _ready() -> void:
	...
	card_navigator.set_title("loot_table.item_details")
	card_navigator.card_changed.connect(_on_card_changed)
	card_navigator.closed.connect(_on_card_navigator_closed)
	item_detail_card.sell_requested.connect(_on_detail_sell_requested)
	item_detail_card.equip_requested.connect(_on_detail_equip_requested)
...
func set_rows(rows: Array[Dictionary]) -> void:
	...
	if card_navigator.visible:
		...
		if cur_id == null or not _rows_by_id.has(str(cur_id)):
...
func _update_card_configuration() -> void:
	if item_detail_card != null:
		var detail_actions_visible := action_presentation == ActionPresentation.DETAIL
		item_detail_card.configure(false, show_sell and detail_actions_visible, show_equip and detail_actions_visible)
...
func _open_detail_for(item_id: String) -> void:
	...
	if row.is_empty():
		return
	var id_list := table.get_displayed_row_ids()
	if id_list.is_empty():
		id_list = _rows_by_id.keys()
	var return_target: Control = view_button if view_button.visible else direct_view_button
	_update_card_configuration()
	card_navigator.open(id_list, item_id, return_target)
...
func _refresh_card(id: String) -> void:
	var row: Dictionary = _rows_by_id.get(id, {})
	item_detail_card.set_item(id, row)


func _on_card_navigator_closed(last_id: Variant) -> void:
	if last_id != null and str(last_id) != "":
		_selected_row_id = str(last_id)
		table.select_row(_selected_row_id)
		_refresh_action_state()
...
func _handle_sell(item_id: String) -> void:
	...
	if int(row.count) <= 1:
		if GameSession.sell_item(item_id, 1):
			sold.emit()
			if card_navigator.visible:
				if not _rows_by_id.has(item_id):
					card_navigator.close()
				else:
