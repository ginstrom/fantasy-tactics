...
## Wound-tier thresholds/colors/badge glyphs/pulse timing and the tier-
## classification function itself live in wound_visuals.gd, shared with
## unit_info_panel.gd -- see that file's own doc comment for why this is a
## separate, purely visual concept from unit_info_panel.gd's
## _wound_tier_key() text tiers.
...
func _build_row(index: int, adventurer_id: String) -> Button:
	...
	var tier: String = WoundVisuals.tier(unit.health, unit.max_health) if unit != null else WoundVisuals.TIER_SLAIN
	...
	health_bar_fill.color = WoundVisuals.HEALTH_BAR_COLORS.get(tier, WoundVisuals.HEALTH_BAR_COLORS[WoundVisuals.TIER_HEALTHY])
	health_bar_fill.set_meta("pulsing", tier == WoundVisuals.TIER_CRITICAL)
	...
	wound_badge.visible = tier != WoundVisuals.TIER_HEALTHY
	wound_badge.text = WoundVisuals.WOUND_BADGE_GLYPHS.get(tier, "")
...
## inline in _build_row(). Each row is freshly built (never reused across
## refresh() calls -- see refresh()'s remove_child()/queue_free() above), so
## there is never a prior tween on this exact node to kill; WoundVisuals.
## apply_pulse() is called with existing_tween = null accordingly.
func _start_critical_pulses() -> void:
	for row in rows.get_children():
		...
		if fill != null and fill.get_meta("pulsing", false):
			WoundVisuals.apply_pulse(fill, true, null)
