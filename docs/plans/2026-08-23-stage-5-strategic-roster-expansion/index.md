...
| 5 | [05-specializations.md](05-specializations.md) | Deliver root specializations one balanced branch at a time, including the deferred Rogue only after its counterplay is accepted. | `feat/stage-5-specializations` | Step 4 merged and a specialization order is approved | Merged 2026-08-23/24 — all four D4-approved branches shipped in order (Knight, Archer, Battle Mage, Paladin), each independently reviewed and manually signed off before merging; Rogue remains deferred per plan |
| 6 | [06-multi-party-strategy.md](06-multi-party-strategy.md) | Audit and replace the one-active-party assumption with independently travelling, selectable parties and bounded time escalation. | `feat/stage-5-multi-party` | Step 5 merged and multi-party/time choices approved | Merged 2026-08-24 — D5 approved (cap of 2 at Guild Hall level 3, config-backed threat-star escalation counter, click-order battle tie-break); four independent review passes each found and fixed a real bug (battle-claim leak, arrival-visibility design gap, stale-panel wrong-party-battle bug, selection-bounce regression), fourth pass clean; manually signed off |
| 7 | [07-stage-5-exit-gate.md](07-stage-5-exit-gate.md) | Prove save-safe, deterministic, comprehensible interaction among all delivered Stage 5 slices and obtain user signoff. | `test/stage-5-exit-gate` | Steps 1–6 merged | Merged 2026-08-24 — one end-to-end journey test (`test_stage_5_strategic_roster_expansion.gd`) composes every D1-D5 mechanic in a single real campaign run (Watchtower/quest intel, Dodge/Parry/Cover/Opportunity-Attack, Mage Sleep + resistance counter, promoted Knight counterplay, two-party travel + battle-claim tie-break, and a mid-route `CampaignSnapshot` checkpoint over all of it); found and fixed a real bug along the way (`CampaignSim._build_player_units()` never threaded a promoted adventurer's `specialization` field, so any fielded promoted Knight/Archer/Paladin would fail scenario validation as an "unknown perk"); independent review found no issues; manually signed off (commit `d97b067`, merge commit `2534ff1`) |
...
**Stage 5 is complete as of 2026-08-24.** All five criteria above are demonstrated by `test_stage_5_strategic_roster_expansion.gd`'s single composed journey plus each step's own deterministic scenario coverage: 2149/2149 tests passing, 5/5 representative campaign-sim seeds unchanged from the Stage 4 baseline, clean headless editor import, clean `git diff --check`, and manual `make play` signoff. Rogue specialization remains deferred per Step 5's own scope; the known "loot in transit" per-party attribution limitation from Step 6 remains accepted and documented (see `decision-ledger.md`'s D5 row) for a future step to pick up if it proves to matter in play.
