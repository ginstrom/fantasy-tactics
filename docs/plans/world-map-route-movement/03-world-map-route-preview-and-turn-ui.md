...
4. In `world_map.gd`, store transient `hover_route: Array[Vector2i]` only. Handle `InputEventMouseMotion` while the party is selected: convert the cursor to a tile, calculate `build_route()`, and redraw. Clear hover state when deselecting, exiting bounds, committing/moving, or leaving the map.
5. Render hover route when non-empty, otherwise `GameSession.get_deployed_party_route()`, using non-interactive translucent segments. At its final point render a non-interactive label equal to remaining route length. Keep the current legal movement-range highlights.
...
Run `make play`. Deploy a party, preview from settlement to camp and confirm an estimate of 8 turns. Click the camp to commit without movement. Click the party once to move one step manually; End Turn must only increment the turn. End seven further turns: each moves one tile, reduces the estimate, and clears the route on arrival. Confirm normal camp entry is still selected-party then second click.
