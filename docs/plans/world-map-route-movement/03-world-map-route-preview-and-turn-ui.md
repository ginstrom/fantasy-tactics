...
1. Add failing scene/HUD tests for `HUD/EndTurnButton`, `HUD/TurnLabel`, and `HUD/Hint`; test that the handler increments the displayed/session turn, moves exactly one unspent route step, persists it, and cannot auto-move after a manual step. Test that the committed destination marker is the click target for manual movement and that clicking the party does not move. Test route/estimate helper state, not pixels: selected mouse hover yields a route and a committed route uses its remaining length as the arrival estimate.
...
Run `make play`. Deploy a party, preview from settlement to camp and confirm an estimate of 8 turns. Click the camp to commit without movement. Click the camp movement target again to move one step manually; clicking the party must not move it. End Turn must only increment the turn. End seven further turns: each moves one tile, reduces the estimate, and clears the route on arrival. Confirm normal camp entry is still selected-party then second click.
