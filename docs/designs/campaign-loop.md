...
| Target party level at Ogre entry | 4–7 | Re-baselined after stationary, unit-transparent missile targeting: representative seeds enter at 4.4–5.6; `test_campaign_sim.gd` preserves headroom around that range |
| Allowed gold/resource range | `gold_earned` 800–1500 per representative seed; total `hp_recovered` (summed across a run's `objective_records`) 150–400 | Re-measured by `make campaign-sim` after stationary, unit-transparent missile targeting (`gold_earned` 1031–1127, `hp_recovered` total 249–284) and locked as a regression floor/ceiling with headroom by `test_campaign_sim.gd`'s `test_representative_seeds_earn_gold_and_recover_hp_within_the_measured_baseline_range()` |
