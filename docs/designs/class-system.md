...
This is the class design for the full Fantasy Tactics experience. **Implemented**
describes live compatibility behaviour; **To implement** is an accepted design
requirement; and **Decision pending** has no locked delivery contract yet. See
the [design status legend](README.md#implementation-status-legend). The
campaign roadmap orders Warrior, Scout, and Cleric as the first three-class
composition, but Scout reconnaissance details and the Cleric/Temple slice are
still decision pending; see [Borderlands Campaign
Loop](campaign-loop.md#decision-pending-design-work).
...
| Root class | Party role | Tactical emphasis | Implementation status |
|---|---|---|---|
| Warrior | Front line | Health, Might, Guard | Implemented foundation; expand only through its approved slices. |
| Scout | Ranged pressure and reconnaissance | Accuracy, Action Points | **To implement**; reconnaissance details are decision pending. |
| Cleric | Sustain and protection | Health, Guard, Resistance | **To implement**; exact slice is decision pending. |
| Mage | Control and burst | spell Accuracy, Action Points | **To implement**. |
